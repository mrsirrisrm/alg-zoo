"""
Create animations for multiple position pairs.
V2: Added final hidden state activations as a subfigure.
"""
...
def create_animation(pos1, pos2, output_name):
    """Create animation for a specific position pair."""

    print(f"\n{'='*70}")
    print(f"Creating animation for positions ({pos1}, {pos2})")
    print(f"{'='*70}")

    # Create output directory
    frames_dir = Path(f"docs/animation_frames_{output_name}")
    ...
    for s_mag in s_mags:
        ...
        states_fwd = run_stepwise([(pos1, M_val), (pos2, s_mag)])
        all_states.extend([s for s in states_fwd[1:]])

        # Reverse: S@pos1, M@pos2
        states_rev = run_stepwise([(pos1, s_mag), (pos2, M_val)])
    ...
    xlim = (all_pca[:, 0].min() - 2, all_pca[:, 0].max() + 2)
    ylim = (all_pca[:, 1].min() - 2, all_pca[:, 1].max() + 2)
    ...
    def render_frame(frame_num, s_mag, mode, total_frames):
        ...
        if mode == "forward":
            states = run_stepwise([(pos1, M_val), (pos2, s_mag)])
            title = f"Forward: M@{pos1} (1.0), S@{pos2} ({s_mag:.2f})"
            target = pos2
            m_pos_actual = pos1
        else:
            states = run_stepwise([(pos1, s_mag), (pos2, M_val)])
        ...
        # 4. Final hidden state activations (NEW)
        ax = axes[0, 3]
        final_h = states[-1]
        bar_colors = ['steelblue' if h > 0 else 'lightgray' for h in final_h]
        ...
        info_text += f"S magnitude: {s_mag:.2f}\nM magnitude: {M_val:.1f}\n"
        info_text += f"Ratio M/S: {M_val/s_mag:.2f}" if s_mag > 0 else ""
    ...
    output_mp4 = f"docs/offset_animation_{output_name}.mp4"
    output_gif = f"docs/offset_animation_{output_name}.gif"
